...
%(fields_list)s""") % {
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        res =  self._read_format(fnames=fields, load=load)
        if isinstance(res, dict):       
            res = self._sanitize_read_output(res)
        elif isinstance(res, list):
            res = [self._sanitize_read_output(vals) for vals in res]
        return res
    ...
    def _sanitize_read_output(self, vals):
        """Sanitize output values for char and text fields."""
        for fname, value in vals.items():
            field = self._fields.get(fname)
            if field and field.type in ('char', 'text'):
                vals[fname] = self._sanitize_html(value)
        return vals
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None, **read_kwargs):
        ...
        if result:
            if isinstance(result, dict):
                result = self._sanitize_read_output(result)
            elif isinstance(result, list):
                result = [self._sanitize_read_output(vals) for vals in result]
        if len(result) <= 1:
            return result
